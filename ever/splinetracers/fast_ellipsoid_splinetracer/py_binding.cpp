...
public:
  ...
  void add_primitives(const torch::Tensor &means, const torch::Tensor &scales,
    ...
    TORCH_CHECK(features.size(0) == numPrimitives,
                "All inputs (features) must have the same 0 dimension")
    TORCH_CHECK(densities.size(0) == numPrimitives,
                "All inputs (densities) must have the same 0 dimension")
    TORCH_CHECK(features.size(2) == 3, "features must have 3 channels. (N, (max_sh_degree + 1)**2), 3")
    TORCH_CHECK(features.size(1) <= 16, "Max sh degree must be 3 or less")
    model.features_size = features.size(1);
  ...
  void set_features(const torch::Tensor &features) {
    TORCH_CHECK(features.size(0) == model.num_prims,
                "All inputs (features) must have the same 0 dimension");
    model.features = reinterpret_cast<float *>(features.data_ptr());
  }
};
...
public:
  torch::Tensor states, touch_count, iters;
  size_t num_prims;
  size_t num_rays;
...
public:
  ...
  py::dict trace_rays(const fesPyGas &gas, size_t sh_degree, const torch::Tensor &ray_origins,
    ...
    image_out = torch::zeros({(long)num_rays, 4},
                         torch::device(device).dtype(torch::kFloat32));
    ...
    forward.trace_rays(gas.gas.gas_handle, sh_degree, num_rays,
                       reinterpret_cast<float3 *>(ray_origins.data_ptr()),
                       reinterpret_cast<float3 *>(ray_directions.data_ptr()),
                       reinterpret_cast<void *>(image_out.data_ptr()),
                       tmin, tmax,
    ...
    return py::dict("image"_a = image_out,
