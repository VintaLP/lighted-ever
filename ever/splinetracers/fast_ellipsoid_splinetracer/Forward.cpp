...
void Forward::trace_rays(
    const OptixTraversableHandle &handle, const size_t sh_degree,
    ...
    uint *iters,
    uint *touch_count, SplineState *last_state,
  ...
  CUDA_CHECK(cudaSetDevice(device));
  {
    ...
    params.features_size = model->features_size;
...
Forward::Forward(const OptixDeviceContext &context, int8_t device,
...

  {
    ...
    params.features.size = model.num_prims;
