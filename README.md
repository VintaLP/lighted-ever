...
#### Dependencies
- Requires OptiX 7.6, which must be downloaded from NVIDIA's [website](https://developer.nvidia.com/designworks/optix/downloads/legacy).


Build an `ever` Docker image:

```
git clone --recursive https://github.com/half-potato/ever_training
cd ever_training
cp -r <path to local optix> optix/
docker build . -t ever 
python docker_build_ever.py
```
Train:
```
python docker_train.py -s <path to dataset> -m <path to output> ...
```
After training, to visualize, do:
```
python docker_visualize.py -s <path to dataset> -m <path to output>
```
and start the SIBR viewer in a different terminal, see [Interactive Viewers](#interactive-viewers).
## Install Locally

#### Dependencies
- OptiX 7.6, which must be downloaded from NVIDIA's [website](https://developer.nvidia.com/designworks/optix/downloads/legacy). This is downloaded and placed somewhere on your computer, then use `export OptiX_INSTALL_DIR=...` to set the variable to that location.
...
- SlangTorch. The version MUST be 1.3.4 or the quality will silently degrade. This will be installed later with pip.
```bash
git clone --recursive https://github.com/half-potato/ever_training
cd ever_training
...
conda create -n ever python==3.10
conda activate ever
# adjust for cuda version, pytorch3d supports torch up to 2.4.1
...
```
Make sure OptiX and Slang can be found:
```
export OptiX_INSTALL_DIR=<path to optix>
export PATH="<path to slang>/bin:$PATH"
```
Build:
```bash
pip install pip==23.3 # newer pip seems to fail installing pytorch3d
...
```
python train.py -s <path to COLMAP or NeRF Synthetic dataset> -m <path to output>
```
For visualization see [Interactive Viewers](#interactive-viewers).
